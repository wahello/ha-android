...
@VisibleForTesting val TIMEOUT_NO_SERVER_FOUND = 5.seconds

@VisibleForTesting val DELAY_AFTER_FIRST_DISCOVERY = 5.seconds
...
internal class ServerDiscoveryViewModel @Inject constructor(val searcher: HomeAssistantSearcher) : ViewModel() {
    ...
    private fun discoverInstances() {
        viewModelScope.launch {
            Timber.e("Hello? $coroutineContext")
            try {
                searcher.discoveredInstanceFlow().collect { instanceFound ->
                    Timber.e("Instance found $instanceFound")
                    ...
                    _discoveryFlow.update {
                        when (it) {
                            ...
                            is ServerDiscovered -> ServersDiscovered(listOf(it, serverDiscovered))
                    ...
                    // TODO we might want to change the logic and not having any delay and just keep the modal above the list of servers.
                    // Validate with design team since most of the time the user will only have once server it might give a better UX like this
                    Timber.e("current context = $coroutineContext")
    ...
    fun onDismissOneServerFound() {
        // TODO depending on the logic we want to apply this function might need to be revisited
        _discoveryFlow.update {
